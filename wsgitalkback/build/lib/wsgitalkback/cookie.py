...
Copyright (c) Sphere Inc
HyperText Transfer Protocol (RFC2616 4.2 http://www.ietf.org/rfc/rfc2616.txt)
RFC 822 - Simple things have been spec-ed it seems in this 
...
class cookie: 
	...
	def setexpiry(self,t):
		self.nvp[html_cookies.enumforkey("expires")]= time.strftime(cookie.timeformat,t)
...
class html_cookies:
	...
	def fromrequest(cls, environ):
		""" generate from request 
		    note - if this survies delete the make in cookies 
			- idea there is simply to come to life with request object as input
		"""
		if not environ:
			return
